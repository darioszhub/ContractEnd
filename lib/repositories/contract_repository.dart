...
class ContractRepository {
  ...
  Future<List<Contract>> getAll() async {
    ...
    return rows.map((row) {
      return Contract(
        ...
        codagent: row['codagent'] as String?,
        clientType: row['clientType'] as String?,
        mercCategory: row['mercCategory'] as String?,
        invoicePeriod: row['invoicePeriod'] as String?,
        meterPower: (row['meterPower'] as num?)?.toDouble(),
        annualVolume: (row['annualVolume'] as num?)?.toDouble(),
        offerType: row['offerType'] as String?,
        variableSpreadNew: (row['variableSpreadNew'] as num?)?.toDouble(),
        variableSpreadOld: (row['variableSpreadOld'] as num?)?.toDouble(),
        currentManager: row['currentManager'] as String?,
        acquisitionDate: row['acquisitionDate'] as String?,
        previousManager: row['previousManager'] as String?,
        expirationNoticeDate: row['expirationNoticeDate'] as String?,
  ...
  Future<int> insert(Contract contract) async {
    ...
    return await db.insert('contracts', {
      ...
      'codagent': contract.codagent,
      'clientType': contract.clientType,
      'mercCategory': contract.mercCategory,
      'invoicePeriod': contract.invoicePeriod,
      'meterPower': contract.meterPower,
      'annualVolume': contract.annualVolume,
      'offerType': contract.offerType,
      'variableSpreadNew': contract.variableSpreadNew,
      'variableSpreadOld': contract.variableSpreadOld,
      'currentManager': contract.currentManager,
      'acquisitionDate': contract.acquisitionDate,
      'previousManager': contract.previousManager,
      'expirationNoticeDate': contract.expirationNoticeDate,
